...
function isOriginAllowed(origin) {
  ...
  try {
    ...
    // 4. Check explicit allowed origins list
    if (allowedOrigins.has(origin)) {
      return true;
    }

    // 5. Allow if just host matches allowedOrigins (relaxed check)
...
app.use(
  cors({
    ...
    allowedHeaders: ['Content-Type', 'Authorization', 'x-device-id', 'x-user-id', 'x-user-name'],
  })
);
...
app.post('/api/remote/accept', (req, res) => {
  ...
  // If targetUserId is anonymous (generated by socket), we trust the bearer of sessionId
  if (
...
io.use((socket, next) => {
  try {
    ...
    if (token && token !== 'null' && token !== 'undefined') {
      const decoded = jwt.decode(token) || {};
      userId = String(decoded.id || decoded._id || decoded.userId || `anon-${socket.id}`);
    }
